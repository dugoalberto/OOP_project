...
#include "SpedizioneInternazionalePage.h"
#include <QVBoxLayout>
#include <QGroupBox>
#include <QComboBox>
#include <QDialog>
...
SpedizioneInternazionalePage::SpedizioneInternazionalePage(SpedizioneInternazionale *spe, bool toEdit, QWidget *parent) : HierachyPageInterface(spe, parent) {
    ...
    else connect(btnConferma, &QPushButton::clicked, this, &SpedizioneInternazionalePage::ModificaSlot);

    lytMain->addLayout(lowerRow);
}


void SpedizioneInternazionalePage::CreaSlot() {
    if( mittente->ConvalidaInput() &
        ...
        SpedizioneInternazionale* nuova = new SpedizioneInternazionale(-1,
                                                               mittente->getAddress(),
...
void SpedizioneInternazionalePage::ModificaSlot() {
    if( mittente->ConvalidaInput() &
        destinatario->ConvalidaInput() &
        package->ConvalidaInput()
        ){
        ...
        emit HierachyPageInterface::ModificaSignal(new SpedizioneInternazionale(obj->getTrakingNumber(),
                                                                                 mittente->getAddress(),
                                                                                 destinatario->getAddress(),
                                                                                 package->getPackage(),
                                                                                 new Stato(possibiliStati->currentText().toStdString(),
